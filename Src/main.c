...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
		...
		angle = (int)((float)AS5600_RawAngle()*360.0/4096);
		
		elapsed = HAL_GetTick() - time1;
		time1 = 0;
  }
  /* USER CODE END 3 */
}
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOB, DIR_Pin|STEP_Pin|ENBL_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : DIR_Pin STEP_Pin ENBL_Pin */
  GPIO_InitStruct.Pin = DIR_Pin|STEP_Pin|ENBL_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
